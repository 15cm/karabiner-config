  ...
  def gen_rules_hook_before
    ...
    def movement_rule_hook()
      ...
      move_key_maps = [
        ...
        ['y', [['a', ['control']]]],
        ['o', [['e', ['control']]]],
        ['semicolon', [['left_arrow',['option']]]],
        ['quote', [['right_arrow', ['option']]]],
        ['comma', [['left_arrow', ['option', 'shift']], ['delete_or_backspace', []]]],
        ['period', [['right_arrow', ['option', 'shift']], ['delete_or_backspace', []]]],
      ]
      # (from_key_code, to_key_code, apps)
      move_app_specific_key_maps = [
        ['y', 'home', ['^com\\.tdesktop\\.Telegram$']],
        ['o', 'end', ['^com\\.tdesktop\\.Telegram$']],
      ]
      @layer_keymap_filter += (move_key_maps + move_app_specific_key_maps).map { |k, _| k }
      ...
      move_manipulators = move_key_maps.map do |f, t| {
        ...
        to: t.map do | key, mods | {
          key_code: key,
          modifiers: (mods if !mods.empty?)
        }.reject{ |k, v| v.nil? }
        end
      }
      end

      move_app_specific_manipulators = move_app_specific_key_maps.map do |f, t, c| {
        conditions: [
          @cond_is_in_this_layer,
          {
            "bundle_identifiers": c,
      ...
      @rules << Rule.gen(
        "#{@layer_description_prefix}: app specific movements",
        move_app_specific_manipulators
      )
      @rules << Rule.gen(
        "#{@layer_description_prefix}: general movements",
        move_manipulators
      )
    end
